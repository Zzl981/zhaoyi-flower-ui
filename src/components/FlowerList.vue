<template>
    <div class="common-layout">
        <el-container>
            <el-header>
                <el-breadcrumb separator="/">
                    <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
                    <el-breadcrumb-item>列表页</el-breadcrumb-item>
                </el-breadcrumb>
            </el-header>
            <el-container>
                <el-aside width="200px">
                    <div class="navigation">
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-effect"></span>
                                <h4>送礼</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-texture"></span>
                                <h4>鲜花</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-color"></span>
                                <h4>用途</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-sort"></span>
                                <h4>更多</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-price"></span>
                                <h4>金钱</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                        <div class="item">
                            <div class="title">
                                <span class="ico ico-num"></span>
                                <h4>礼物</h4>
                            </div>
                            <ul class="item-list list-inline">
                                <li>
                                    <a href="javascript:;">爱情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">友情鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">生日鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">问候长辈</a>
                                </li>
                                <li>
                                    <a href="javascript:;">回报老师</a>
                                </li>
                                <li>
                                    <a href="javascript:;">探病慰问</a>
                                </li>
                                <li>
                                    <a href="javascript:;">道歉鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">婚庆鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">祝贺鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">哀思鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">商务鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">开业花篮</a>
                                </li>
                                <li>
                                    <a href="javascript:;">自选鲜花</a>
                                </li>
                                <li>
                                    <a href="javascript:;">港澳台送花</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </el-aside>
                <el-main>
                    <el-image style="display:inherit;width: 1048px;margin-top: -20px;" :src="image"></el-image>
                    <div class="grid-wrapper">
                        <div class="grid-item" v-for="item in list" :key="item">
                            <el-link @click="GoToDetail(item.id)">
                                <div class="grid-panel">
                                    <div class="img-box">
                                        <el-image class="list-image" :src=item.image></el-image>
                                    </div>
                                    <div class="info-cont">
                                        <div class="price">
                                            <span class="price-sign">¥</span>
                                            <span class="price-num">{{item.price}}</span>
                                        </div>
                                        <div class="title">
                                            <span class="product-title">{{item.title}}</span>
                                        </div>
                                    </div>
                                </div>
                            </el-link>
                        </div>
                    </div>
                </el-main>
            </el-container>
        </el-container>
    </div>
</template>
 <script lang="ts" setup>
import { getLoveFlowers, getBirthdayFlowers, getFriendFlowers, getWeddingFlowers } from '../http/index'
import { ref, onMounted } from 'vue'
import { defineProps } from 'vue'
import { useRouter } from 'vue-router'
const router = useRouter();
// 接收调用组件传入的参数
const props = defineProps({
    image: String,
    type: Number,
})
const list = ref();
onMounted(async () => {
    switch (props.type) {
        case 1:
            list.value = (await getLoveFlowers()).data
            break;
        case 2:
            list.value = (await getBirthdayFlowers()).data
            break;
        case 3:
        list.value = (await getFriendFlowers()).data
            break;
        case 4:
            list.value = (await getWeddingFlowers()).data
            break;
    }
})
const GoToDetail = (currId: Number) => {
    const sss = ref()
    sss.value = currId;
    router.push({ path: '/detail', query: { id: sss.value, type: props.type } });
}
</script>

<style lang="scss">
.el-breadcrumb {
    margin-top: 20px;
}

.ico-flower,
.ico-ysh,
.ico-Plant,
.ico-effect,
.ico-texture,
.ico-sort,
.ico-sort-gift,
.ico-price,
.ico-color,
.ico-num,
.ico-city,
.ico-brand {
    width: 30px;
    height: 40px;
    background-image: url(//localhost:8080/images/common_z.png);
    display: block;
    background-repeat: no-repeat;
}

.navigation {
    margin-bottom: 20px;
    padding: 0 9px;
    border: 1px solid #ccc;

    .item {
        position: relative;
        padding: 12px 0;
        height: 175px;
        border-bottom: 1px solid #d9d9d9;

        .title {
            position: absolute;
            width: 32px;
            margin-top: 50px;
            text-align: center;

            .ico-effect {
                background-position: -160px -234px;
            }

            .ico-texture {
                background-position: -120px -234px;
            }

            .ico-color {
                background-position: 0px -234px;
            }

            .ico-sort {
                background-position: -80px -234px;
            }

            .ico-price {
                background-position: -40px -234px;
            }

            .ico-num {
                background-position: -200px -234px;
            }

            h4 {
                font-family: tahoma, arial, "Hiragino Sans GB", "\5FAE\8F6F\96C5\9ED1",
                    sans-serif;
                font-size: 14px;
                font-weight: bold;
                margin: 0;
                color: #ff6a00;
            }
        }

        .item-list {
            margin-bottom: 0;
            margin-left: 50px;
            padding-left: 0;
            list-style: none;

            li {
                font-size: 12px;
                line-height: 22px;
                width: 50%;
                margin: 0;
                margin-right: -3px;
                padding: 0;

                a {
                    color: #737373;
                    text-decoration: none;
                }
            }
        }
    }
}

.grid-wrapper {
    width: 1048px;

    .grid-item {
        float: left;
        margin-bottom: 20px;
        padding: 10px;
        border: 1px solid transparent;
        background-color: #fff;

        .list-image {
            width: 240px;
            height: 255px;
        }

        .info-cont {
            .price {
                text-align: initial;
                margin-top: 10px;
                margin-bottom: 8px;
                color: #ff6a00;

                .price-sign {
                    font-family: arial;
                }

                .price-num {
                    font-weight: 700;
                    line-height: 18px;
                    margin-right: 10px;
                }
            }

            .title {
                font-size: 12px;
                line-height: 18px;
                overflow: hidden;
                height: 36px;
                margin-bottom: 8px;
                text-align: justify;

                a {
                    color: #737373;
                    text-decoration: none;
                }
            }
        }
    }
}
</style>